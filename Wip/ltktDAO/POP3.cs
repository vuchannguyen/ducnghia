...
namespace ltktDAO
{
    public class POP3Client : IDisposable
    {
        ...
        public Emails FetchEmail(int emailId)
        {
            if (IsResponseOk(SendCommand("TOP " + emailId + " 0")))
                return new Emails(ReadLines());
            else
                return null;
        }

        public List<Emails> FetchEmailList(int start, int count)
        {
            List<Emails> emails = new List<Emails>(count);

            for (int i = start; i < (start + count); i++)
            {
                Emails email = FetchEmail(i);
    ...
    public class Emails
    {
        ...
        public Emails(string emailText)
        {
